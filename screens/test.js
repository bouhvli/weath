...
const WeatherApp = () => {
  ...
  const [dayweathertest, setDayweathertest] = useState(null);
  ...
  const fetchWeatherData = () => {
    ...
    const apiweather = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=weather_code&timezone=${timezone}`;
    fetch(apiUrl)
    ...
    fetch(apiweather)
      .then(res => res.json())
      .then(data => {
        setDayweathertest(data);
      })
      .catch(error => {
        console.error('Error fetching data from weather day:', error);
        setDayweathertest(null);
      });
  };
